...
int main(int argc, char **argv)
{
    ...
    for (int round = 0; round < repetitions; round++)
    {
        ...
        MPI_Status status;
        int offset=1;
        for (offset=1; offset<number_of_processes-my_rank && my_rank%(offset*2)==0;offset=offset*2){
        ...
        if (my_rank!=0){
            int masterOffset=1;
            for (masterOffset=1;my_rank%(masterOffset*2)==0;masterOffset=masterOffset*2){}
            MPI_Send(&result,1,MPI_DOUBLE,my_rank-masterOffset,tag,MPI_COMM_WORLD);
